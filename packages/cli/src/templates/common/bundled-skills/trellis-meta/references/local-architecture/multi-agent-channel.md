...
- **Platform integration layer**: channel runtime is platform-neutral. It does not depend on `.claude/`, `.codex/`, `.pi/`, or any other platform directory. The adapters that normalize provider output (Claude `stream-json`, Codex `app-server`, Pi `--mode rpc`) live inside the Trellis CLI binary, not in the project.
